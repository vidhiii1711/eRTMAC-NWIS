...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      // 1. Get REAL risk predictions from your Node backend -> her FastAPI model
      try {
        const riskData = await getRiskPrediction(currentWell.wellId);
        setRisk(riskData); // { wellId, timestamp, depth, prediction: {...} }
      } catch (err) {
      ...
      // 2. Build Early Warning by checking similar wells for high-similarity matches
      //    (unchanged — still using your working Similar Wells feature)
      try {
        const similarData = await getSimilarWells(currentWell.wellId, 5);
        const closeMatches = similarData.similarWells.filter((w) => w.similarityScore >= 60);
        if (closeMatches.length > 0) {
  ...
  return (
    <div>
      <div className="page-title">
        Well: {currentWell.wellId}{" "}
        <span style={{ fontSize: 13, color: "#16a34a" }}>● {currentWell.status}</span>
      </div>
      <div className="page-subtitle">
        Field: {currentWell.field} · Depth: {currentWell.totalDepth} m · Formation:{" "}
        {currentWell.formation || "—"}
      </div>

      {/* EARLY WARNING BANNER — unchanged */}
      {earlyWarning?.triggered && (
        <div className="card" style={{ background: "#fef2f2", border: "1px solid #fecaca" }}>
          <h3 style={{ color: "#dc2626" }}>⚠️ Early Warning — Approaching a Historically Risky Zone</h3>
          <p style={{ fontSize: 14 }}>
            {earlyWarning.count} similar well{earlyWarning.count > 1 ? "s" : ""} found at
            comparable depth/formation. Closest match:{" "}
            <strong>{earlyWarning.topMatch.wellId}</strong> (similarity{" "}
            {earlyWarning.topMatch.similarityScore}).
          </p>
        </div>
      )}
      ...
      {/* HISTORICAL DOCUMENT SEARCH — unchanged, still mock */}
      <div className="card">
        <h3>Historical Document Search</h3>
        <p style={{ fontSize: 13, color: "#64748b", marginBottom: 8 }}>
          e.g. "What happened around 2900m?"
        ...
        <form onSubmit={handleAsk} style={{ display: "flex", gap: 8 }}>
          <input
            ...
            placeholder="Ask about nearby well history..."
          />
          <button className="primary" type="submit" disabled={asking}>
            {asking ? "Searching..." : "Ask"}
          </button>
